...
class EthereumIdentitySDK {
  ...
  async addKey(to, publicKey, privateKey, transactionDetails) {
    ...
    const { data } = new Interface(Identity.interface).functions.addKey(
      key,
      MANAGEMENT_KEY,
      ECDSA_TYPE
    );
  ...
  async addKeys(to, publicKeys, privateKey, transactionDetails) {
    ...
    const { data } = new Interface(Identity.interface).functions.addKeys(
      keys,
      keyRoles,
      keyTypes
    );
  ...
  async removeKey(to, address, privateKey, transactionDetails) {
    ...
    const { data } = new Interface(Identity.interface).functions.removeKey(
      key,
      MANAGEMENT_KEY
    );
    const message = {
  ...
  async execute(contractAddress, message, privateKey) {
    ...
    const nonce = parseInt(
      await this.getLastExecutionNonce(contractAddress, wallet),
      10
    );
    message.nonce = nonce;
  ...
  getExecutionNonce(emittedEvents) {
    const [eventTopic] = new Interface(
      Identity.interface
    ).events.ExecutionRequested.topics;
  ...
  async identityExist(identity) {
    ...
    if (
      identityAddress &&
      codeEqual(
        Identity.runtimeBytecode,
        await this.provider.getCode(identityAddress)
      )
    ) {
  ...
  subscribe(eventType, identityAddress, callback) {
    if (['AuthorisationsChanged'].includes(eventType)) {
      return this.relayerObserver.subscribe(
        eventType,
        identityAddress,
        callback
      );
    } else if (['KeyAdded', 'KeyRemoved'].includes(eventType)) {
      return this.blockchainObserver.subscribe(
        eventType,
        identityAddress,
        callback
      );
