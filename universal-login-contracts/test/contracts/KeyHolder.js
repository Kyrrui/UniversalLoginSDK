...
describe('Key holder', async () => {
  ...
  describe('Create', async () => {
    ...
    it('Should return the purpose', async () => {
      expect(await identity.keyHasPurpose(managementKey, MANAGEMENT_KEY)).to.be
        .true;
      expect(await identity.keyHasPurpose(managementKey, ACTION_KEY)).to.be
        .false;
      expect(await identity.keyHasPurpose(managementWalletKey, MANAGEMENT_KEY))
        .to.be.true;
      expect(await identity.keyHasPurpose(actionKey, MANAGEMENT_KEY)).to.be
        .false;
      expect(await identity.keyHasPurpose(actionKey, ACTION_KEY)).to.be.false;
      expect(await identity.keyHasPurpose(actionWalletKey, ACTION_KEY)).to.be
        .true;
    });
  });
  ...
  describe('Add key', async () => {
    ...
    it('Should not allow to add existing key', async () => {
      await expect(identity.addKey(managementKey, MANAGEMENT_KEY, ECDSA_TYPE))
        .to.be.reverted;
    });

    it('Should emit KeyAdded event successfully', async () => {
      await expect(addActionKey())
    ...
    it('Should not allow to add new key with unknown key', async () => {
      await expect(
        fromUnknownWallet.addKey(unknownWalletKey, MANAGEMENT_KEY, ECDSA_TYPE)
      ).to.be.reverted;
    });

    it('Should not allow to add key with action key', async () => {
      await expect(
        fromActionWallet.addKey(unknownWalletKey, MANAGEMENT_KEY, ECDSA_TYPE)
      ).to.be.reverted;
    });
  });
  ...
  describe('Get key', async () => {
    ...
    it('Should return keys by purpose correctly', async () => {
      const actualManagementKeys = await identity.getKeysByPurpose(
        MANAGEMENT_KEY
      );
  ...
  describe('Remove key', async () => {
    ...
    it('Should emit KeyRemoved event successfully', async () => {
      expect(await isActionKey()).to.be.true;
      await expect(identity.removeKey(actionKey, ACTION_KEY))
    ...
    it('Should not allow to remove key with unknown key', async () => {
      expect(await isActionKey()).to.be.true;
      await expect(fromUnknownWallet.removeKey(actionKey, ACTION_KEY)).to.be
        .reverted;
    });

    it('Should not allow to remove key with action key', async () => {
      await expect(fromActionWallet.removeKey(actionKey, ACTION_KEY)).to.be
        .reverted;
    });
    ...
    it('Should not allow to remove last management key', async () => {
      ...
      expect(await identity.keyHasPurpose(managementWalletKey, MANAGEMENT_KEY))
        .to.be.false;
      await expect(identity.removeKey(managementKey, MANAGEMENT_KEY)).to.be
        .reverted;
    });

    it('Should not allow to remove key with invalid purpose', async () => {
      await expect(identity.removeKey(actionWalletKey, MANAGEMENT_KEY)).to.be
        .reverted;
    });
  });
});
